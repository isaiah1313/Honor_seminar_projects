...
\begin{frame}{質点の時間発展}
    \begin{block}{ミンコフスキー空間での質点の運動の記述}
        質点の運動はミンコフスキー空間での曲線(世界線)として表される。\\
        弧長パラメータ
        $\mathrm{d}s = \sqrt{\tensor{g}{_\mu_\nu}\mathrm{d}x^\mu\mathrm{d}x^\nu}=\sqrt{\mathrm{d}x_\nu\mathrm{d}x^\nu}$
        を用いて$x^\mu(s)$と媒介変数表示して曲線を表す。
    \end{block}

    \begin{tikzpicture}[scale = 0.15]
        \draw[->,>=stealth,thick](-10,0)--(10,0)node[right]{$x^i$}; %x^i軸
        \draw[->,>=stealth,thick](0,-10)--(0,10)node[above]{$x^0$}; %x^0軸
        \draw[thick](-7,-10) to[out=90-10,in=270-30] (0,0);
        \draw[thick](0,0) to[out = 90-30,in =-90](5,10);
    \end{tikzpicture}
